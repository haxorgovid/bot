...
function verif_data(){
    ...
    -d 'address=jl+aceh+asoy+geboy+'$(rand)'&city=339&confirm_password_chg=&decide_csrf='$1'&email=&fav_brand1=500019272&fav_brand2=500020640&interest=Travel&interest_raw=Travel&new_password_chg=&old_password_chg=&password=&phone_number=08953407244729&postalcode=0&province=30&security_answer=rihana&security_question=500001007&stop_subscribe_email_promo=false' \
    -b $cok --cookie-jar $cok \
    | jq .data.status | tr -d \"
}
get_video(){
    curl -s "https://www.marlboro.id/article/$1" -b $cok --cookie-jar $cok |  grep -Po "(?<=data-ref=\"https://www.marlboro.id/maze-of-decision/article/).*?(?=\"\><div)" | sort -fsu > vid.txt
    cat vid.txt > salinan_vid.txt
#     curl -s "https://www.marlboro.id/" -b $cok --cookie-jar $cok | grep -Po "(?<=data-ref\=\"https://www.marlboro.id/discovered/article/).*?(?=\")" | shuf | head -6 > vid.txt; cat vid.txt > salinan_vid.txt
}
submit_video(){
...
echo "start from ${start} | Ends at: $(date +%R) | `pwd`"